...
class PlayOfflineMenu(ui.UI):
	def __init__(self, screen):
		...
		self.enemiesValue = label.Text([my.SCREEN_HALF_WIDTH + 50, (my.SCREEN_HEIGHT // 8)], "5", fontsize=18)
		self.addWidget(self.enemiesValue)
		self.addWidget(label.Text([my.SCREEN_HALF_WIDTH - 10, (my.SCREEN_HEIGHT // 8)], "Inimigos", fontsize=18))
		self.addWidget(slider.SliderBar(partial(self.sliderEnemies), [my.SCREEN_HALF_WIDTH, 100], (200, 5),
							 min=1, max=10, step=1,
							 initial=5,
...
class OptionsMenu(ui.UI):
	def __init__(self, screen):
		...
		self.addWidget(
			slider.SliderBar(partial(self.sliderFps), [my.SCREEN_HALF_WIDTH, my.SCREEN_HEIGHT//4], (300, 5), min=20, max=100, step=1,
							 initial=my.FPS,
							 image_slider=ui.IMAGES['slider'],
							 image_pointer=ui.IMAGES['slider_pointer']))
